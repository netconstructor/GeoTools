...
public class VersionedJDBCFeatureStore<T extends SimpleFeatureType, F extends SimpleFeature>  extends ContentFeatureStore implements VersioningFeatureStore<SimpleFeatureType, SimpleFeature> {
	...
	public VersionedJDBCFeatureStore(ContentEntry entry,Query query) throws IOException {
        super(entry,query);
        
        delegate = new VersionedJDBCFeatureSource<SimpleFeatureType, SimpleFeature>( entry, query ) {
            @Override
