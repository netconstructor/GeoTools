...
public class WKTReader {
     ...
     *        returned by the stream should be L_PAREN (the beginning of
    ...
    private Point readPointText() throws IOException, ParseException {
        ...
        Point point = new PointImpl(positionFactory.createDirectPosition(this
                .getPreciseCoordinate().getCoordinates())); // primitiveFactory.createPoint(this.getPreciseCoordinate().getCoordinates());
        getNextCloser();
        return point;
    }
    ...
    private Surface readPolygonText() throws IOException, ParseException {
        ...
        if (nextToken.equals(EMPTY)) {
            return new SurfaceImpl((SurfaceBoundary) null); // this.primitiveFactory.createSurface((SurfaceBoundary)
                                                            // null);
        }
        ...
        SurfaceBoundary sfb = new SurfaceBoundaryImpl(crs, shell, holes); // this.primitiveFactory.createSurfaceBoundary(shell,
                                                                          // holes);
    ...
    private OrientableCurve createCurve(Coordinate[] aCoords) {
        ...
        for (int i = 0; i < aCoords.length; i++) {
            points.add(this.positionFactory.createPosition(this.positionFactory
                    .createDirectPosition((aCoords[i].getCoordinates()))));
        }
